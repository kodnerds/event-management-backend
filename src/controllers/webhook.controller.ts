...
export const paystackWebook = async (req: Request, res: Response) => {
  try {
    ...
    if (event.event === 'charge.success') {
      ...
      await rsvpRepository.updateStatus(payment.rsvp, RsvpStatus.REGISTERED);
      await showRepository.decrementAvailableTickets(payment.shows);

      return res.status(HTTP_STATUS.OK).json({ message: 'Payment successful' });
    } else if (event.event === 'charge.failed') {
      await paymentRepository.updateStatus(payment, PaymentStatus.FAILED);
      return res.status(HTTP_STATUS.OK).json({ message: 'Payment failed' });
    }
